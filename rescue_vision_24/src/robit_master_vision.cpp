#include "../include/rescue_vision_24/robit_master_vision.hpp"
...
RobitLabeling::RobitLabeling(const cv::Mat& Img, const unsigned int& nThreshold, const unsigned int& nNeighbor)
{
  if (!m_recBlobs.empty())
    m_recBlobs.clear();
...
const unsigned int RobitLabeling::__check_four_neighbor(cv::Point& start_pt, cv::Point& end_pt)
{
  ...
  for (;;)
  {
    ...
    if (present_pt.x != 0 && m_isVisited[nowIdx + Visit[LEFT]] == false && m_Image.data[nowIdx + Visit[LEFT]] != 0)
    {
    ...
    if (present_pt.x != m_width - 1 && m_isVisited[nowIdx + Visit[RIGHT]] == false &&
    ...
    if (present_pt.y != 0 && m_isVisited[nowIdx + Visit[UP]] == false && m_Image.data[nowIdx + Visit[UP]] != 0)
    {
    ...
    if (present_pt.y != m_height - 1 && m_isVisited[nowIdx + Visit[DOWN]] == false &&
...
const unsigned int RobitLabeling::__check_eight_neighbor(cv::Point& start_pt, cv::Point& end_pt)
{
  cv::Point present_pt = start_pt;
  m_area = 1;
  const int Visit[8] = { -1, -m_width - 1, -m_width, -m_width + 1, 1, m_width + 1, m_width, m_width - 1 };

  for (;;)
  {
    unsigned int nowIdx = (present_pt.y * m_width) + present_pt.x;

    if (present_pt.x != 0 && m_isVisited[nowIdx + Visit[W]] == false && m_Image.data[nowIdx + Visit[W]] != 0)
    {
    ...
    if (present_pt.x != 0 && present_pt.y != 0 && m_isVisited[nowIdx + Visit[NW]] == false &&
    ...
    if (present_pt.y != 0 && m_isVisited[nowIdx + Visit[N]] == false && m_Image.data[nowIdx + Visit[N]] != 0)
    {
    ...
    if (present_pt.x != m_width - 1 && present_pt.y != 0 && m_isVisited[nowIdx + Visit[NE]] == false &&
    ...
    if (present_pt.x != m_width - 1 && m_isVisited[nowIdx + Visit[E]] == false && m_Image.data[nowIdx + Visit[E]] != 0)
    {
    ...
    if (present_pt.x != m_width - 1 && present_pt.y != m_height - 1 && m_isVisited[nowIdx + Visit[SE]] == false &&
    ...
    if (present_pt.y != m_height - 1 && m_isVisited[nowIdx + Visit[S]] == false && m_Image.data[nowIdx + Visit[S]] != 0)
    {
    ...
    if (present_pt.x != 0 && present_pt.y != m_height - 1 && m_isVisited[nowIdx + Visit[SW]] == false &&
...
RobitLineDetect::RobitLineDetect() : RobitLabeling()
{
}

RobitLineDetect::RobitLineDetect(const Mat& Img, const unsigned int& nThreshold, const unsigned int& nNeighbor)
