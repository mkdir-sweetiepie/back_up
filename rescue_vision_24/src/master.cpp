...
void MASTER::update()
{
  ...
  switch (movement_save)
  {
    ...
    case 2:
      ...
      break;
    default:
      cv::putText(clone_mat, "STOP", cv::Point(640 - 200, 30), 0.5, 1, cv::Scalar(0, 0, 0), 2, 8);
      break;
  }
...
void MASTER::img_detect_movement(Mat &input_img)
{
  Mat gray_img;
  Mat binary_img;
  ...
  {
    // cv::rectangle(Divided_Image__Rotation_Direction[count__Rotation_Direction], BOX.m_recBlobs[0], Scalar(0, 0, 255),
    // 5, 8);
    cv::rectangle(input_img, BOX.m_recBlobs[0], Scalar(0, 0, 255), 5, 8);
    MovementBox_center = Point(BOX.m_recBlobs[0].x + (BOX.m_recBlobs[0].width / 2),
                               BOX.m_recBlobs[0].y + (BOX.m_recBlobs[0].height / 2));
    circle(movement_Find_center_img, Point(MovementBox_center.x, MovementBox_center.y), 1, Scalar(255, 255, 255), -1);
  }
  //---============================
  movement_count += 1;

  Point Center = Point(movement_Find_center_img.rows / 2, movement_Find_center_img.cols / 2);
  Point Vec1, Vec2;
  ...
  else if (movement_count > 15)
  {
    ...
    {
      // cout << endl << endl << save_image_position__Rotation_Direction << ": " << degree << ": CW" << endl;
      movement_save = 1;
    }
    else if (degree < 0 && degree < -0.4)
    {
      // cout << endl << endl << save_image_position__Rotation_Direction << ": " << degree << ": CCW" << endl;
      movement_save = 2;
    }
    else
    {
      // cout << endl << endl << save_image_position__Rotation_Direction << ": " << degree << ": STOP" << endl;
      movement_save = 0;
    }
  ...
  //     cv_bridge::CvImage(std_msgs::Header(), sensor_msgs::image_encodings::MONO8, binary_img).toImageMsg());
}

Mat MASTER::region_of_interest(Mat input_img)
{
