...
public:
  RobitLabeling();
  RobitLabeling(const cv::Mat& Img, const unsigned int& nThreshold, const unsigned int& nNeighbor = 4);
...
public:
  RobitLineDetect();
  RobitLineDetect(const cv::Mat& Img, const unsigned int& nThreshold, const unsigned int& nNeighbor = 4);
...
public:
  ...
  const vector<vector<KeyLine>>& getKeyLinesVec() const
  {
    return _mKeylinesvec;
  }
